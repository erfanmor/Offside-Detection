...
class OffsideClass():
        ...
        for r in results:
            for box in r.boxes:
                ...
                if conf > 0.4:  # Only objects with confidence above 40%
                    cv.rectangle(self.__image, (x1, y1), (x2, y2), (255, 0, 0), 2)
    ...
    def auho_line_detection(self, max_width=500):
    ...
    def draw_line(self, vanishing_point, max_width=800):
        ...
        while True:
            ...
            if point_from_mouse[0] is not None:
                cv.line(image, tuple(map(int, point_from_mouse[0])), tuple(map(int, vanishing_point)), (0, 0, 255), 2)

            if point_from_mouse[1] is not None:
                cv.line(image, tuple(map(int, point_from_mouse[1])), tuple(map(int, vanishing_point)), (0, 255, 0), 2)
